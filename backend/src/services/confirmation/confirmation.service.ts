...
export async function confirmIncident(
  ...
  try {
    ...
    if (typeof (db as any).runTransaction === "function") {
      result = await (db as any).runTransaction(
        async (transaction: {
    ...
    // Sync confirmationCount to hospitalEmergencyRequests
    try {
      const hReqSnap = await db
        .collection("hospitalEmergencyRequests")
        .where("emergencyId", "==", emergencyId)
        .get();
      for (const hDoc of hReqSnap.docs) {
        await hDoc.ref.set(
          { confirmationCount: result.confirmationCount, updatedAt: new Date().toISOString() },
          { merge: true },
        );
      }
      const directHReq = db.collection("hospitalEmergencyRequests").doc(emergencyId);
      const directSnap = await directHReq.get();
      if (directSnap.exists) {
        await directHReq.set(
          { confirmationCount: result.confirmationCount, updatedAt: new Date().toISOString() },
          { merge: true },
        );
      }
