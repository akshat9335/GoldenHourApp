...
class MedicineService {
  ...
  public seedHospitalInventory(hospitalId: string, hospitalName = "Emergency Hospital Center"): MedicineStockItem[] {
    const standardMedicines = [
      { name: "Atropine Injection (0.6mg)", category: "Emergency / Resuscitation", dosageForm: "1ml Ampoule", qty: 45, status: "AVAILABLE" as StockStatus },
      { name: "Adrenaline / Epinephrine (1mg)", category: "Emergency / Resuscitation", dosageForm: "1ml Ampoule", qty: 28, status: "AVAILABLE" as StockStatus },
      { name: "Aspirin (300mg / Dispersible)", category: "Cardiac / Antiplatelet", dosageForm: "Tablet", qty: 250, status: "AVAILABLE" as StockStatus },
      ...
      { name: "Pantoprazole IV (40mg)", category: "Gastrointestinal / Antacid", dosageForm: "Vial", qty: 95, status: "AVAILABLE" as StockStatus },
      { name: "Ceftriaxone Injection (1g)", category: "Antibiotic", dosageForm: "Vial", qty: 60, status: "AVAILABLE" as StockStatus },
      { name: "Salbutamol Respirator Solution", category: "Respiratory / Bronchodilator", dosageForm: "Respules", qty: 20, status: "AVAILABLE" as StockStatus },
    ];

    const seeded: MedicineStockItem[] = [];
    const now = new Date().toISOString();

    for (const m of standardMedicines) {
      const id = `${hospitalId}_${m.name.replace(/[^a-zA-Z0-9]/g, "_")}`;
      const item: MedicineStockItem = {
        id,
        hospitalId,
        hospitalName,
      ...
      if (firestore && process.env.NODE_ENV !== "test") {
        firestore.collection("medicineInventory").doc(id).set(item, { merge: true }).catch(() => {});
      }
    }
  ...
  public async searchMedicines(query?: string, hospitalId?: string): Promise<MedicineStockItem[]> {
    ...
    if (firestore && process.env.NODE_ENV !== "test") {
      try {
        ...
        const snap = await collQuery.limit(100).get();
    ...
    // If hospital queried specifically and has 0 items, auto-seed default emergency essentials
    if (results.length === 0 && hospitalId) {
      return this.seedHospitalInventory(hospitalId);
    }
  ...
  public async addMedicine(data: Partial<MedicineStockItem> & { hospitalId: string; medicineName: string }): Promise<MedicineStockItem> {
    const now = new Date().toISOString();
    const id = data.id || `${data.hospitalId}_${data.medicineName.replace(/[^a-zA-Z0-9]/g, "_")}`;
    const item: MedicineStockItem = {
      id,
      hospitalId: data.hospitalId,
      hospitalName: data.hospitalName || "Hospital Medical Center",
      ...
      quantity: typeof data.quantity === "number" ? data.quantity : 50,
      updatedAt: now,
    };
    ...
    if (firestore && process.env.NODE_ENV !== "test") {
      try {
        await firestore.collection("medicineInventory").doc(id).set(item, { merge: true });
      } catch (err) {
        console.warn("[MedicineService] Firestore add error:", err);
      }
    }
  ...
  public async deleteMedicine(id: string): Promise<boolean> {
    this.inMemoryInventory.delete(id);
    if (firestore && process.env.NODE_ENV !== "test") {
      try {
        await firestore.collection("medicineInventory").doc(id).delete();
      } catch (err) {
        console.warn("[MedicineService] Firestore delete error:", err);
      }
    }
    return true;
  }
