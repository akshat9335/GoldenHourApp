...
router.post("/register", requireAuth, (req, res, next) => doctorController.registerDoctor(req, res, next));
router.get("/", (req, res, next) => doctorController.searchDoctors(req, res, next));

// Authenticated doctor self-management
router.get("/me", requireAuth, requireRole("DOCTOR"), (req, res, next) => doctorController.getMyDoctorProfile(req, res, next));
router.patch("/me", requireAuth, requireRole("DOCTOR"), (req, res, next) => doctorController.updateMyDoctorProfile(req, res, next));
...
router.patch("/:id/verify", requireAuth, requireRole("ADMIN"), (req, res, next) => doctorController.verifyDoctor(req, res, next));
